...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        # print("self.gradient['W']", self.gradient['W'].shape)
        self.gradient['b'] = np.zeros((1, output_D))
        # print("self.gradient['b']", self.gradient['b'].shape)



    def forward(self, X):
        # print("X shape :", np.transpose(X).shape)
        # print("self.params['W'] shape : ", np.transpose(self.params['W']).shape)
    ...
    def backward(self, X, grad):
        ...
        # print(">> X : ", np.transpose(X).shape, "grad : ", grad.shape)
        ...
        # print("backward_output shape : ", backward_output.shape)
...
class relu:
    ...
    """
        ...
        self.mask is an attribute of relu. You can use it to store things (computed in the forward pass) for the use in the backward pass.
            ==> compute forward, then store something in self.mask
    """
    ...
    def forward(self, X):
        ...
        # store gradients
        derivatives = X > 0.0
        self.mask = derivatives.astype(float)
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        # print("self.mask shape : ", self.mask.shape, "g shape : ", grad.shape)
        backward_output = self.mask * grad
        # print("X shape : ", X.shape, "b_o shape : ", backward_output.shape)
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ...
                    ###################################################################################################
                    # use momentum[module_name + '_' + key] as it is the "get info" for the  momentum dictionary
