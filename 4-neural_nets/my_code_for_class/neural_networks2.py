...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        self.params = dict()
        self.gradient = dict()
        print("input_D by output_D : ", input_D, "x", output_D, "\n")
        ###############################################################################################
        ...
        mu = 0
        s_d = 0.1
        self.params['W'] = np.random.normal(mu, s_d, (input_D, output_D))
        # print("self.params['W'] : ", self.params['W'].shape)
        self.params['b'] = np.random.normal(mu, s_d, (1, output_D))
        # print("self.params['b'] : ", self.params['b'].shape)
        ...
        # print("self.gradient['W'].shape : ", self.gradient['W'].shape)
    ...
    def forward(self, X):
        ...
        # x_0 = X[0].shape
        # print("X[0].shape transpose : ", np.transpose(x_0))
        ...
        ################################################################################
        # print("self.params['W'].shape : " , self.params['W'].shape)
        # print("X^T by W: ", np.transpose(X[0]).shape, " x ", self.params['W'].shape)
        # x_dot_W = np.dot(np.transpose(X[0]), self.params['W'])
        # print("x_dot_W.shape : " , np.transpose(x_dot_W).shape)
        # print("self.params['b'].shape : " , self.params['b'].shape)
        # forward_output = np.add(x_dot_W, self.params['b'])
        # print("forward_output.shape : " , forward_output.shape)
        # return forward_output

        # print("X by W : " , X.shape, "x", self.params['W'].shape)
        x_dot_W = np.dot(X, self.params['W'])
        # print("x_dot_W.shape : " , x_dot_W.shape, "x", self.params['b'].shape)
    ...
    def backward(self, X, grad):
        ...
        #################################################################################################
        print("b4 - self.params['W'].shape : ", self.params['W'].shape)
        print("b4 - self.gradient['W'].shape : ", self.gradient['W'].shape)
        print("X.shape : ", X.shape)
        print("grad.shape : ", grad.shape)
        self.gradient['W'] = np.dot(np.transpose(X), grad)
        print("after - self.gradient['W'].shape : ", self.gradient['W'].shape)
...
class relu:
    ...
    def forward(self, X):
        ...
        # store gradients if it's > 0
        derivatives = X > 0.0
        self.mask = np.array(derivatives, dtype=float)
...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ### backward pass ###
            grad_a1 = model['loss'].backward(a2, y)
            ######################################################################################
